...
class Auth extends Autenticar{
    ...
    private function accederToken(string $proc, string $idUsuario, string $tkRef = ""){
        $sql = $proc === "modificar" ? "CALL modificarToken" : "CALL verificarTokenR";
        $sql .= "(:idUsuario, :tkRef)";
    ...
    private function generarToken(string $idUsuario,string $nombre, string $telefono){
    ...
    public function iniciar(Request $request, Response $response, $args){
        ...
        if ($datos = $this->autenticar($body->usuario, $body->contraseña)) {
            $tokens = $this->generarToken($body->usuario, $datos['nombre'], $datos['telefono']);
